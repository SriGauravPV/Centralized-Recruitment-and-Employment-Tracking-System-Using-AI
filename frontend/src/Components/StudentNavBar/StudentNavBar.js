...
const StudentNavBar = () => {
  ...
  const navItems = [
    { label: 'Dashboard', path: '/home/studentWebPage' },
    { label: 'Create Resume', path: 'http://localhost:3000' },
    { label: 'Active Drives', path: '/home/active-drives' },
    { label: 'Mock Interview', path: 'http://localhost:5173' }
  ];
          ...
          <Nav className="mx-auto">
            {navItems.map((item, index) => (
              ...
              >
                <Nav.Link
                  onClick={() =>
                    item.path.startsWith('http') ? window.location.href = item.path : navigate(item.path)
                  }
